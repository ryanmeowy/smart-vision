...
1. `false`：当前应用层 RRF（基线实现）。
2. `true`：ES 原生 `retriever.rrf` 路径。

## 9.2 本地验证

1. 保持 `native-enabled=false`，执行一组固定查询，记录结果顺序与高亮表现。
2. 改为 `native-enabled=true`，重启服务后执行同一组查询。
3. 对比以下维度：
   - 返回条数与耗时
   - TopN 文档变化
   - 高亮覆盖率变化
   - rerank 指标波动（保持原指标名不变）

## 9.3 灰度建议

1. 首批仅在测试环境开启 `native-enabled=true`。
2. 预发环境按低流量实例灰度，保留至少 1 个发布周期观察。
3. 灰度期间记录业务样本查询（高频词、长尾词、OCR 强依赖查询）。

## 9.4 回滚

1. 配置回滚：将 `native-enabled` 设置回 `false`。
2. 重启后立即恢复旧路径，无需数据迁移。
3. 若出现异常，优先执行配置回滚，再做问题定位。
