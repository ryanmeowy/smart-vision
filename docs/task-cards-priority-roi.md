...
| 9 | `TC-19 搜索高亮片段（部分已实现）` | P1 | 7/10 | 提升结果可解释性与命中证据可读性 | ES 查询增加 highlight（`ocrContent/tags/fileName`）；后端返回 `highlight` 片段；UI 优先展示后端片段，缺失时回退前端关键词高亮；补充测试与文档 | 复杂语义解释系统；多语言分词与同义词体系改造；复杂摘要生成 | 用户可看到 ES 命中片段；`highlight` 为空时页面平滑降级；核心查询回归通过 | 1-2 天 |
